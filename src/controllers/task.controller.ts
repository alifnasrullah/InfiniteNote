...
export const getTasksForToday = async (
  ...
  try {
    const userId = request.user
    const todaysISODate = new Date().toISOString().split('T')[0]; // Format tanggal ISOString
    const tomorrowISODate = new Date(new Date().getTime() + 24 * 60 * 60 * 1000).toISOString().split('T')[0]; // Tambah 1 hari
    const tasks = await Task.find({
      user: userId,
      date: { $gte: todaysISODate, $lt: tomorrowISODate },
    });
